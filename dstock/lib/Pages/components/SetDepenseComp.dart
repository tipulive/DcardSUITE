...
class SetDepenseComp extends StatefulWidget {
  const SetDepenseComp({super.key});
...
class _SetDepenseCompState extends State<SetDepenseComp> {
  ...
  stockCount()async
  {
    ...
    var resultData=(await StockQuery().stockCount(Topups(uid:"${orderData[0]}"),QuickBonus(uid:productCode,qty:"$qtyProduct",subscriber:"StockName",status:"status",description:"Delivered"), User(uid: "UidTransport",name:"refName"))).data;
