...
"uid"=>$uidOwner
    ...
    public function AdminChangePermission(Request $request){
        if(Auth::check())
        {
            ...
            if(Auth::user()->platform==$platform1)
            {
                ...
                $check=DB::select("select name,created_at,uid,status,CompanyName,subscriber,permission from admins");
                if($check)
             {
              return response([
                "myuid"=>Auth::user()->uid,
            ...
            if(Auth::user()->platform==$platform1)
            {
                ...
                if($check)
             {
              ...
              return response([
                ...
                $check=DB::select("select name,created_at,uid,status,CompanyName,subscriber,permission from admins where subscriber=:subscriber limit 10",array(
